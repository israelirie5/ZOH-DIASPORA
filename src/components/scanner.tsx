...
export function Scanner({ eventId }: { eventId: string }) {
  ...
  async function open() {
    ...
    try {
      ...
      controls.current = await reader.decodeFromConstraints(
        ...
        async (value, _error, scannerControls) => {
          if (!value) return;
          ...
          const response = await fetch("/api/check-in/resolve", {
            method: "POST",
  ...
  return (
    <div className="space-y-5">
      ...
      <button
        onClick={open}
        className="min-h-14 w-full bg-[#ffd21c] font-black text-[#073f36]"
      >
        OUVRIR LA CAMÉRA
      </button>
      <p className="text-center text-sm">Placez le QR Code dans le cadre</p>
      {error && (
        <p role="alert" className="bg-red-100 p-4 text-red-900">
          {error}
        </p>
      )}
      {result && (
        ...
        >
            ...
            {result.status === "valid"
              ...
              : result.status === "used"
                ...
                : result.status === "wrong_event"
                  ? "TICKET POUR UN AUTRE ÉVÉNEMENT"
          ...
          {result.participant && (
            <p className="mt-2">
              ...
              <br />
              {result.participant.ticket_number}
            </p>
          )}
          {result.status === "valid" && (
            <button
              onClick={confirm}
              className="mt-4 min-h-12 w-full bg-[#176652] px-4 font-bold text-white"
            >
              VALIDER L’ENTRÉE
            </button>
          )}
